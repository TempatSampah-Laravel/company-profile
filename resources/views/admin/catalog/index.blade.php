@if(user_akses2('catalog', Auth::user()->role_id)->view ?? 0 =='1')
                ...
                <div class="row justify-content-between">
                    ...
                    <div class="col-auto">
                        <div class="text-lg-end my-1 my-lg-0">
                            ...
                            <a href="{{ route('catalog.create') }}" class="btn btn-primary"> <i class="fas fa-plus"></i><span class="mx-1">Create New Certificate</span></a>
                            @endif
                        ...
                        <table id="basic-datatable" class="table dt-responsive nowrap w-100">
                            <thead>
                                <tr style="background: #f3f7f9">
                                    ...
                                    @if(user_akses2('catalog', Auth::user()->role_id)->update ?? 0 =='1' OR user_akses2('catalog', Auth::user()->role_id)->delete ?? 0 =='1')
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <img src="{{ asset('upload/catalog/'. $catalog->image) }}" class="rounded" width="50px" height="50px" alt="image">
                                    </td>
                                    <td>{{ $catalog->name_en }}</td>
                                    <td>{{ Str::limit($catalog->description_en, 50, $end='...') }}</td>

                                    @if(user_akses2('catalog', Auth::user()->role_id)->update ?? 0 =='1' OR user_akses2('catalog', Auth::user()->role_id)->delete ?? 0 =='1')
                                    <td class="text-center">
                                        <form onsubmit="return confirm('Are you sure ?');"
                                        action="{{ route('catalog.destroy', $catalog) }}"
                                        method="POST">

                                        @if(user_akses2('catalog', Auth::user()->role_id)->update ?? 0 =='1')
                                        <a href="{{ route('catalog.edit', $catalog) }}" title="Edit" class="btn btn-info btn-sm">
                                            <i class="far fa-edit"></i>
                                        </a>
                                        @endif

                                        @if(user_akses2('catalog', Auth::user()->role_id)->delete ?? 0 =='1')
